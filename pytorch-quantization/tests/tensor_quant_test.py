...
import pytest
import torch
from pytorch_quantization import cuda_ext, tensor_quant
...
np.random.seed(
    123456
)  # seed 1234 causes 1 number mismatch at 6th decimal in one of the tests
...
class TestTensorQuant:
    ...
    def test_per_channel_scale(self):
        ...
        amax_x_torch = (
            ...
            * torch.max(
                torch.max(
                    torch.max(x_torch, dim=1, keepdim=True)[0], dim=2, keepdim=True
                )[0],
                dim=3,
                keepdim=True,
            )[0]
    ...
    def test_backward(self):
        ...
        np.testing.assert_array_equal(
            float_quant_x.grad.cpu().numpy(), x.grad.cpu().numpy()
        )

    def test_unsigned(self):
        x_np = np.random.rand(1023).astype("float32")
        x_torch = torch.Tensor(x_np)
        quant_x_np = test_utils.quant_np(
            x_np, np.max(np.abs(x_np)), num_bits=9, fake=False
        )
        quant_x_torch, _ = tensor_quant.tensor_quant(
    ...
    def test_overflow_fp16(self):
        ...
        with pytest.raises(ValueError, match="scale is too large for FP16"):
            quant_x_torch, scale = tensor_quant.tensor_quant(
                x_torch, torch.tensor(1e-4).cuda().half(), 8, False
            )
    ...
    def test_clip_gradient(self):
        ...
        loss.backward()
        np.testing.assert_array_equal(
            x.grad.cpu().numpy() != 0, x_in_range.cpu().numpy()
        )
    ...
    def test_full_range(self):
        ...
        quant_x_np = test_utils.quant_np(
            x_np, amax, num_bits=9, fake=False, narrow_range=False
        )
        quant_x_torch, _ = tensor_quant.tensor_quant(
...
class TestFakeTensorQuant:
    ...
    def test_per_channel_scale(self):
        ...
        np.testing.assert_array_almost_equal(
            quant_x_torch.cpu().numpy(), quant_x_np, decimal=2
        )
        if verbose:
            mismatches = np.where(
    ...
    def test_unsigned(self):
        ...
        x_torch = torch.Tensor(x_np).cuda()
        quant_x_np = test_utils.quant_np(
            x_np, np.max(np.abs(x_np)), num_bits=9, fake=True
        )
        quant_x_torch = tensor_quant.fake_tensor_quant(
            x_torch, torch.max(torch.abs(x_torch)), 8, True
        )
    ...
    def test_cuda_ext(self):
        ...
        for num_bits in [3, 4, 5, 7, 8, 11]:
            for unsigned in [True, False]:
                test_utils.compare(
                    cuda_ext.fake_tensor_quant(
                        x_torch, torch.max(torch.abs(x_torch)), num_bits, unsigned
                    ),
        ...
        test_utils.compare(
            cuda_ext.fake_tensor_quant(
                x_torch_fp16, torch.max(torch.abs(x_torch_fp16))
            ),
            tensor_quant.fake_tensor_quant(
                x_torch_fp16, torch.max(torch.abs(x_torch_fp16))
            ),
    ...
    def test_cuda_ext_inplace(self):
        ...
        x_np_fp16 = np.random.rand(1023).astype("float16")
        x_torch_fp16 = torch.Tensor(x_np_fp16).cuda().half()
        quant_x_np_fp16 = test_utils.quant_np(
            x_np_fp16, np.max(np.abs(x_np_fp16)), fake=True
        )
        cuda_ext.fake_tensor_quant_(x_torch_fp16, torch.max(torch.abs(x_torch_fp16)))
        np.testing.assert_array_almost_equal(
            x_torch_fp16.cpu().numpy(), quant_x_np_fp16, decimal=2
        )

    def test_cuda_ext_tiny_amax(self):
        x_torch = torch.rand(2, 3, 4, device="cuda")
        amax = (
            torch.tensor([1.0, 1.0e-26, 1.0], device="cuda").unsqueeze(-1).unsqueeze(1)
        )
    ...
    def test_overflow_fp16(self):
        ...
        assert not (
            torch.isinf(quant_x_torch).any() or torch.isnan(quant_x_torch).any()
        )
    ...
    def test_full_range(self):
        ...
        amax = np.max(np.abs(x_np))
        quant_x_np = test_utils.quant_np(
            x_np, amax, num_bits=9, fake=True, narrow_range=False
        )
        quant_x_torch = tensor_quant.fake_tensor_quant(
            x_torch, torch.max(torch.abs(x_torch)), 8, True, False
        )
    ...
    def test_against_legacy(self, dtype):
        ...
        for num_bits in [3, 4, 5, 7, 8, 11]:
            for unsigned in [True, False]:
                legacy_out = tensor_quant.legacy_fake_tensor_quant(
                    x_torch, amax_torch, num_bits, unsigned
                )
                test_out = tensor_quant.fake_tensor_quant(
                    x_torch, amax_torch, num_bits, unsigned
                )
    ...
    def test_against_legacy_noncontiguous(self):
        ...
        legacy_out = tensor_quant.legacy_fake_tensor_quant(
            x_torch_noncontiguous, amax_torch
        )
...
class TestQuantDescriptor:
    ...
    def test_from_to_dict(self):
        quant_desc_1 = tensor_quant.QuantDescriptor(
            num_bits=2, name="a", fake_quant=True, axis=(1, 2), amax=3.1415926536
        )
...
class TestFakeAffineTensorQuant:
    def test_simple_run(self, verbose):
        x = np.array(
            [-1.0, -13.0, -101.0, -128.0, 0.0, 2.0, 5.0, 13.0, 93.0, 111.0, 127.0],
            dtype=np.float32,
        )
        torch_x = torch.tensor(x).cuda()
        quant_x = tensor_quant.fake_affine_tensor_quant(
            torch_x, torch.min(torch_x), torch.max(torch_x)
        )
