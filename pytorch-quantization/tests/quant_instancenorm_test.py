...
class TestQuantInstanceNorm1D:
    ...
    def test_fake_quant_per_channel(self):
        quant_instancenorm_object = quant_instancenorm.QuantInstanceNorm1d(
            NUM_CHANNELS, affine=True, quant_desc_input=QuantDescriptor(axis=(1))
        )
...
class TestQuantInstanceNorm2D:
    def test_no_quant(self):
        quant_instancenorm_object = quant_instancenorm.QuantInstanceNorm2d(
            NUM_CHANNELS, affine=True
        )
        quant_instancenorm_object.input_quantizer.disable()
    ...
    def test_fake_quant_per_channel(self):
        ...
        quant_input = tensor_quant.fake_tensor_quant(
            test_input,
            torch.abs(test_input)
            .max(0, keepdim=True)[0]
            .max(2, keepdim=True)[0]
            .max(3, keepdim=True)[0],
        )
...
class TestQuantInstanceNorm3D:
    def test_no_quant(self):
        quant_instancenorm_object = quant_instancenorm.QuantInstanceNorm3d(
            NUM_CHANNELS, affine=True
        )
        quant_instancenorm_object.input_quantizer.disable()
