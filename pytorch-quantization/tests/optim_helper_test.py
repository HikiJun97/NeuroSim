...
class TestGroupParameters:
    ...
    def test_lr_momentum_decay(self, resnet18):
        ...
        param_groups = helper.group_parameters(
            resnet18,
            [["conv1.*weight"], ["downsample.*.weight"]],
            lrs,
            momentums,
            weight_decays,
        )
    ...
    def test_raises(self):
        ...
        with pytest.raises(TypeError, match="must match"):
            helper.group_parameters(
                None, [["downsample.1.weight"], ["conv1"]], lrs=[0.1]
            )

        with pytest.raises(TypeError, match="must match"):
            helper.group_parameters(
                None, [["downsample.1.weight"], ["conv1"]], momentums=[0.1]
            )

        with pytest.raises(TypeError, match="must match"):
            helper.group_parameters(
                None, [["downsample.1.weight"], ["conv1"]], weight_decays=[0.1]
            )
