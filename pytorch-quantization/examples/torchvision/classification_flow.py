...
try:
    from train import evaluate, load_data, train_one_epoch, utils
except Exception:
...
def get_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Classification quantization flow script"
    )

    parser.add_argument(
        "--data-dir", "-d", type=str, help="input data folder", required=True
    )
    parser.add_argument(
        "--model-name", "-m", default="resnet50", help="model name: default resnet50"
    )
    parser.add_argument(
        "--disable-pcq",
        "-dpcq",
    ...
    parser.add_argument(
        "--out-dir", "-o", default="/tmp", help="output folder: default /tmp"
    )
    parser.add_argument(
        "--print-freq",
        "-pf",
        type=int,
        default=20,
        help="evaluation print frequency: default 20",
    )
    parser.add_argument(
        "--threshold",
        "-t",
    ...
    parser.add_argument(
        "--batch-size-onnx", type=int, default=1, help="batch size for onnx: default 1"
    )
    ...
    parser.add_argument(
        "--calibrator", type=str, choices=["max", "histogram"], default="max"
    )
    parser.add_argument(
        "--percentile", nargs="+", type=float, default=[99.9, 99.99, 99.999, 99.9999]
    )
    parser.add_argument(
        "--sensitivity", action="store_true", help="Build sensitivity profile"
    )
    parser.add_argument(
        "--evaluate-onnx", action="store_true", help="Evaluate exported ONNX"
    )
...
def main(cmdline_args):
    ...
    for epoch in range(args.num_finetune_epochs):
        ...
        if "print_freq" in inspect.signature(train_one_epoch).parameters:
            train_one_epoch(
                model, criterion, optimizer, data_loader_train, "cuda", 0, 100
            )
        else:
            _args = collections.namedtuple(
                "mock_args",
    ...
    top1_onnx = -1.0
    if (
        export_onnx(model, onnx_filename, args.batch_size_onnx, not args.disable_pcq)
        and args.evaluate_onnx
    ):
        ## Validate ONNX and evaluate
        top1_onnx = evaluate_onnx(
            onnx_filename, data_loader_onnx, criterion, args.print_freq
        )
...
def evaluate_onnx(onnx_filename, data_loader, criterion, print_freq):
    ...
    with torch.no_grad():
        ...
        print(
            "  ONNXRuntime: Acc@1 {top1.global_avg:.3f} Acc@5 {top5.global_avg:.3f}".format(
...
def export_onnx(model, onnx_filename, batch_onnx, per_channel_quantization):
    ...
    dummy_input = torch.randn(
        batch_onnx, 3, 224, 224, device="cuda"
    )  # TODO: switch input dims by model
    try:
        ...
        warnings.warn(
            UserWarning(
                "Per-channel quantization is not yet supported in Pytorch/ONNX RT (requires ONNX opset 13)"
            )
        )
