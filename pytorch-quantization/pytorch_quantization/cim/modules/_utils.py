...
class QuantMixin:
    ...
    def init_cim(self, cim_args, in_features, out_features):
        ...
        if cim_args.mem_states_file:  # external mean and std
            with open(cim_args.mem_states_file, "r") as file:
                ...
                if "mean" in header:
                    ...
                    if n == 0 or (n & (n - 1)) != 0:
                        print(
                            f"The number of states in the external file ({n}) is not a power of 2."
                        )
                        exit(-1)
                    self._cim_args.bitcell = int(math.log2(n))

                    self.mem_mean_gaussian = torch.tensor(
                        means, dtype=torch.float32, device="cuda"
                    )
                    self.mem_std_gaussian = torch.tensor(
                        stds, dtype=torch.float32, device="cuda"
                    )
                    self._cim_args.mem_states = torch.tensor(
                        means, dtype=torch.float32, device="cuda"
                    )
                    if self.mem_std_gaussian.sum() != 0:
                        self.mem_sampling_seed = torch.randint(
                            1, 1000, (self._cim_args.weight_precision,)
                        )
                    else:
        ...
        # Set intermediate states based on on and off state
            ...
            self._cim_args.mem_step = (
                self._cim_args.on_state - self._cim_args.off_state
            ) / (num_states - 1)
            self._cim_args.mem_states = torch.zeros(num_states, device="cuda")
            self._cim_args.mem_states[0] = self._cim_args.off_state
            for s in range(num_states):
                self._cim_args.mem_states[s] = (
                    self._cim_args.off_state + s * self._cim_args.mem_step
                )
        ...
        if self._cim_args.mem_type == "resistive":
            self._cim_args.amp_feedback = 1 / (
                self._cim_args.parallel_read
        ...
        if self._cim_args.mem_type == "capacitive":
            self._cim_args.amp_feedback = self._cim_args.parallel_read * (
                self._cim_args.mem_states[-1] - self._cim_args.mem_states[0]
            )
        ...
        if self._cim_args.parallel_read > self._cim_args.sub_array[0]:
            print(
                "WARNING: parallel read is larger than sub array size, reducing parallel read to match..."
            )
        ...
        self._cim_args.num_arrays_x = math.ceil(
            out_features * self._cim_args.weight_precision / cim_args.sub_array[1]
        )
            ...
            with open(cim_args.output_noise_file, "r") as file:
                ...
                if "output" in header:
                    ...
                    if len(values) > 2 ** (cim_args.adc_precision) + 1:
                        print(
                            "The required 'output' numbers are larger than the ADC precision."
                        )
                        exit(-1)

                    # Convert lists to PyTorch tensors
                    self.output_values = torch.tensor(
                        values, dtype=torch.float32, device="cuda"
                    )
                    if stds:
                        self.output_noise_stds = torch.tensor(
                            stds, dtype=torch.float32, device="cuda"
                        )
                    else:
                        self.output_noise_stds = torch.zeros(0)
                    if means:
                        self.output_noise_means = torch.tensor(
                            means, dtype=torch.float32, device="cuda"
                        )
                    else:
        ...
        # MLC
        self._cim_args.cells_per_weight = math.ceil(
            self._cim_args.weight_precision / self._cim_args.bitcell
        )
        self._cim_args.cells_per_weight = math.ceil(
            self._cim_args.weight_precision / self._cim_args.bitcell
        )
        self._cim_args.base = 2**self._cim_args.bitcell
        self._cim_args.cycles_per_input = math.ceil(
            self._cim_args.input_precision / self._cim_args.dac_precision
        )
    ...
    def init_quantizer(
        ...
        if (
            ...
            or (not quant_desc_adc.fake_quant)
        ):
        ...
        logging.info(
            "Output is %squantized to %d bits in %s with axis %s!",
            "" if not quant_desc_adc.fake_quant else "fake ",
            quant_desc_adc.num_bits,
            self.__class__.__name__,
            quant_desc_adc.axis,
        )
        ...
        if num_adc_quantizers is None:
            ...
            self._adc_quantizers = nn.ModuleList(
                [TensorQuantizer(quant_desc_adc) for _ in range(num_adc_quantizers)]
            )
