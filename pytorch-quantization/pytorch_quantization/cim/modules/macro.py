...
class CIM:
    def simulate_array(self, input2d, weight2d):
        ...
        input2d = torch.cat(
            (
                input2d,
                torch.full(
                    (1, input2d.shape[1]), fill_value=shift_input, device=input2d.device
                ),
            ),
            dim=0,
        )
        ...
        weight2d = torch.cat(
            (
                ...
                torch.full(
                    (weight2d.shape[0], 1),
                    fill_value=shift_weight,
                    device=weight2d.device,
                ),
            ),
            dim=1,
        )
        ...
        num_zeros_y = (
            self._cim_args.num_arrays_y * self._cim_args.parallel_read
            - weight2d.shape[0]
        )
        num_zeros_x = (
            self._cim_args.num_arrays_x * self._cim_args.sub_array[1]
            - weight2d.shape[1]
        )
        ...
        if num_zeros_y > 0:
            input2d = torch.cat(
                (
                    input2d,
                    torch.zeros((input2d.shape[0], num_zeros_y), device=input2d.device),
                ),
                dim=1,
            )
            weight2d = torch.cat(
                (
                    weight2d,
                    torch.zeros(
                        (num_zeros_y, weight2d.shape[1]), device=weight2d.device
                    ),
                ),
                dim=0,
            )
        ...
        input2d = input2d.reshape(
            input2d.shape[0], self._cim_args.num_arrays_y, -1
        ).transpose(0, 1)
        ...
        matmul_out = torch.zeros(
            (input2d.shape[1], weight2d.shape[2]), device=input2d.device
        )
        ...
        for i in range(self._cim_args.cycles_per_input):
            ...
            input = ((input2d.to(torch.int32)) >> self._cim_args.dac_precision * i) & (
                self._cim_args.input_base - 1
            )
            ...
            for j in range(self._cim_args.cells_per_weight):
                ...
                weight = ((weight2d.to(torch.int32)) >> self._cim_args.bitcell * j) & (
                    self._cim_args.base - 1
                )
                ...
                if self._cim_args.hardware:
                    ...
                    out = torch.matmul(
                        input.to(torch.float32), weight.to(torch.float32)
                    )
    ...
    def stuck_at_fault_sample(self, weights):
        ...
        num_cells = (
            weights.shape[0] * weights.shape[1] * self._cim_args.cells_per_weight
        )
        num_stuck = int(
            num_cells * (self._cim_args.rate_stuck_0 + self._cim_args.rate_stuck_1)
        )
        ...
        if self._cim_args.rate_stuck_0 > 0:
            indices_flat_zeros = indices_flat[
                : math.ceil(num_stuck * self._cim_args.rate_stuck_0)
            ]
            ...
            mask_expanded = torch.ones(
                (weights.shape[0], weights.shape[1], self._cim_args.weight_precision),
                dtype=torch.int32,
            )
            for i in range(cells_per_weight):
                if (
                    i == 0
                    and self._cim_args.weight_precision % self._cim_args.bitcell != 0
                ):
                    lsbs = self._cim_args.weight_precision % self._cim_args.bitcell
                    mask_expanded[..., 0:lsbs] = (
                        mask[..., i].unsqueeze(-1).expand(-1, -1, lsbs)
                    )
                else:
                    mask_expanded[
                        ...,
                        i * self._cim_args.bitcell : (i + 1) * self._cim_args.bitcell,
                    ] = (
                        mask[..., i]
                        .unsqueeze(-1)
                        .expand(-1, -1, self._cim_args.bitcell)
                    )
            binary_weights = binary_weights & mask_expanded.to(binary_weights.device)
        if self._cim_args.rate_stuck_1 > 0:
            indices_flat_ones = indices_flat[
                math.ceil(num_stuck * self._cim_args.rate_stuck_0) : num_stuck
            ]
            mask_flat = torch.zeros(num_cells, dtype=torch.int32)
            mask_flat[indices_flat_ones] = 1
            mask = mask_flat.view(weights.shape[0], weights.shape[1], cells_per_weight)
            mask_expanded = torch.zeros(
                (weights.shape[0], weights.shape[1], self._cim_args.weight_precision),
                dtype=torch.int32,
            )
            for i in range(cells_per_weight):
        ...
        weights_sampling = torch.matmul(
            binary_weights.to(torch.float32), bit_significance.to(torch.float32)
        )
    ...
    def conductance_sampling(self, weights, weights_ref, weight_significance):
        ...
        gen = torch.Generator(
            device=self._cim_args.mem_states.device
        )  # seed generator for weight conductance
        gen.manual_seed(
            self.mem_sampling_seed[weight_significance].item()
        )  # fixed seed for the weight conductance
        gen_ref = torch.Generator(
            device=self._cim_args.mem_states.device
        )  # seed generator for reference conductance column
        gen_ref.manual_seed(
            self.mem_sampling_seed[0].item()
        )  # fixed seed for the reference conductance column
        sampled_memory_states = torch.normal(
            mean=self.mem_mean_gaussian[weights],
            std=self.mem_std_gaussian[weights],
            generator=gen,
        )
        sampled_reference_states = torch.normal(
            mean=self.mem_mean_gaussian[weights_ref],
            std=self.mem_std_gaussian[weights_ref],
            generator=gen_ref,
        )
    ...
    def calc_drift(self, memory_states, memory_states_ref, weights):
        ...
        if self._cim_args.detect == 1:  # need to define the sign of v
            ...
            sign = torch.sign(
                torch.add(torch.zeros_like(memory_states), target) - memory_states
            )
            ratio = self._cim_args.t ** (self._cim_args.v * sign)
            sign_ref = torch.zeros_like(memory_states_ref)
            sign_ref = torch.sign(
                torch.add(torch.zeros_like(memory_states_ref), target_ref)
                - memory_states_ref
            )
        ...
        memory_states_ref = torch.clamp(
            (memory_states_ref * ratio_ref), lower_ref, upper_ref
        )
    ...
    def ADC_output(self, inputs, weights, weights_ref, weight_significance):
        ...
        if self._cim_args.output_noise != 0.0:
            ADC_out_ref = torch.matmul(
                inputs.to(torch.float32), weights.to(torch.float32)
            ).to(torch.int32)
            ADC_out_ref = torch.clamp(
                ADC_out_ref, min=0, max=2**self._cim_args.adc_precision
            )
            if self._cim_args.output_noise == -1.0:
                ADC_out = torch.normal(
                    mean=self.output_noise_means[ADC_out_ref],
                    std=self.output_noise_stds[ADC_out_ref],
                )
            elif self._cim_args.output_noise > 0.0:
                ADC_out = torch.normal(
                    mean=ADC_out_ref.to(torch.float32), std=self._cim_args.output_noise
                )
            else:
            ...
            return torch.clamp(
                torch.round(ADC_out),
                min=0.0,
                max=float(2**self._cim_args.adc_precision),
            )
        ...
        if self.mem_sampling_seed.numel() != 0:
            memory_states, memory_states_ref = self.conductance_sampling(
                weights, weights_ref, weight_significance
            )

        # device drifting
        if self._cim_args.v != 0:
            memory_states, memory_states_ref = self.calc_drift(
                memory_states, memory_states_ref, weights
            )

        # read noise for weight cell and reference cell
        if self._cim_args.read_noise > 0:
            memory_states = torch.normal(
                mean=memory_states, std=memory_states * self._cim_args.read_noise
            )
            memory_states_ref = torch.normal(
                mean=memory_states_ref,
                std=memory_states_ref * self._cim_args.read_noise,
            )
        ...
        analog_step = self._cim_args.vdd * (
            self._cim_args.mem_states[1] - self._cim_args.mem_states[0]
        )
        V_step = analog_step * self._cim_args.amp_feedback
        scale = 1 / V_step
        ADC_out = torch.round(scale * V_out)
        ADC_out = torch.clamp(
            ADC_out, min=0.0, max=float(2**self._cim_args.adc_precision)
        )
        return ADC_out
...
def dec2bin(x, num_bits):
    ...
    output = torch.zeros(
        x.shape[0], x.shape[1], num_bits, device=x.device, dtype=torch.int
    )
...
def dec2val(x, num_bits, cells_per_weight, bitcell, base):
    ...
    output = torch.zeros(
        x.shape[0], x.shape[1], cells_per_weight, device=x.device, dtype=torch.int
    )
...
def write_layer(self, input2d, weight2d):
    ...
    rows_per_input = (
        input2d.shape[0] // self._cim_args.batch_size
        if input2d.shape[0] > self._cim_args.batch_size
        else input2d.shape[0]
    )
    input2d = input2d[0:rows_per_input, :].t()

    input_file_name = (
        "./layer_record_"
        + self._cim_args.model
        + "/input_"
        + str(self._cim_args.name)
        + ".csv"
    )
    weight_file_name = (
        "./layer_record_"
...
def save_weight_compare(self, weight_state, weight_significance):
    ...
    if os.path.exists(file_path):
        ...
        if not torch.equal(saved_weight, weight_state):
            raise ValueError(
                f"file: {file_name} 's weight is different to weight_state"
            )
    else:
