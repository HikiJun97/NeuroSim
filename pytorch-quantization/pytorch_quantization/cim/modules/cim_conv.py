...
class _CIMConvNd(torch.nn.modules.conv._ConvNd, macro.CIM, _cim_utils.QuantMixin):
    ...
    def __init__(
        ...
        super(_CIMConvNd, self).__init__(
            in_channels,
            out_channels,
            kernel_size,
            stride,
    ...
    def _load_from_state_dict(self, state_dict, prefix, *args, **kwargs):
        ...
        super(_CIMConvNd, self)._load_from_state_dict(
...
class CIMConv2d(_CIMConvNd):
    ...
    def forward(self, input):
        if self._cim_args.write_network:
            ...
            with open(filename, "a") as f:
                ...
                f.write(
                    f"{input.shape[2]},{input.shape[3]},{input.shape[1]},{self.kernel_size[0]},{self.kernel_size[1]},{self.out_channels},0,{self.stride[0]}\n"
                )
        ...
        if (
            ...
            if self.padding_mode == "circular":
                ...
                if self._cim_args.hook:
                    if (
                        self._cim_args.quant_mode == "adc"
                        and self._input_quantizer._disabled
                    ):
                        _ = self.conv2d(
                            F.pad(quant_input, expanded_padding, mode="circular"),
                            quant_weight,
    ...
    def conv2d(self, input, weight, bias, stride, padding, dilation, groups):
        ...
        H_out = (
            input_shape[-2]
            + 2 * self.padding[0]
            - self.dilation[0] * (weight_shape[-2] - 1)
            - 1
        ) // self.stride[0] + 1
        W_out = (
            input_shape[-1]
            + 2 * self.padding[1]
            - self.dilation[1] * (weight_shape[-1] - 1)
            - 1
        ) // self.stride[1] + 1
