...
def _resolve_upload(
    uploaded, fallback_path: str, target_name: str, run_dir: Path
) -> str:
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        if record_dir.exists():
            for p in record_dir.rglob("*"):
                if p.is_file():
                    zf.write(
                        p, arcname=f"layer_record_{model}/{p.relative_to(record_dir)}"
                    )
...
def run_inference(
    ...
    off_state,
    on_state,
    mem_states_file_path,
    mem_states_upload,
    vdd,
    # Noise / reliability
    read_noise,
    output_noise,
    output_noise_file_path,
    output_noise_upload,
    t,
    v,
    detect,
    ...
    try:
        ...
        mem_states = _resolve_upload(
            mem_states_upload, mem_states_file_path or "", "mem_states.csv", run_dir
        )
        noise_file = _resolve_upload(
            output_noise_upload,
            output_noise_file_path or "",
            "output_noise.csv",
            run_dir,
        )
...
"""
    with gr.Blocks(title="NeuroSim", js=_autoscroll_js) as demo:
        ...
        gr.Markdown(
            "`inference.py` 를 서브프로세스로 호출합니다. `--run_dir` 패치 필요."
        )
        ...
        with gr.Tabs():
            with gr.Tab("Core"):
                ...
                data_path = gr.Textbox(
                    "./datasets/", label="data_path", info="path to saved datasets"
                )
                model_path = gr.Textbox(
                    "./models/", label="model_path", info="path to saved models"
                )
                test_name = gr.Textbox("test", label="test_name", info="test name")
                gpu = gr.Number(
                    0,
                    label="gpu",
                    precision=0,
                ...
                calib_batch_size = gr.Number(
                    128,
                    label="calib_batch_size",
                    precision=0,
                    info="input batch size for calibrating quantization (default: 128)",
                )
                seed = gr.Number(
                    1234, label="seed", precision=0, info="random seed (default: 1)"
                )
            ...
            with gr.Tab("Hardware"):
                ...
                off_state = gr.Textbox(
                    "1e-15",
                    label="off_state (S 또는 F)",
                    info="device off state (conductance (S) or capacitance (F))",
                )
                on_state = gr.Textbox(
                    "1e-14",
                    label="on_state (S 또는 F)",
                    info="device on conductance / capacitance",
                )
                mem_states_file_path = gr.Textbox(
                    "mem_states.csv",
                    label="mem_states_file 경로 (빈 문자열 = on/off만 사용)",
                    info="path to .csv file containing mean and std for each memory state",
                )
                mem_states_upload = gr.File(
                    label="mem_states.csv 업로드 (선택, 우선순위 높음)",
                    file_types=[".csv"],
                )
                vdd = gr.Number(1.0, label="vdd", info="supply voltage")

            with gr.Tab("Noise / Reliability"):
                read_noise = gr.Number(
                    0.0,
                    label="read_noise",
                    info="std of conductance read noise, derive from SPICE or experiment, or use to test reliability",
                )
                output_noise = gr.Number(
                    0.0,
                    label="output_noise (-1 = 파일에서 std 로드)",
                    info="std of output voltage noise, derive from SPICE or experiment, or use to test reliability",
                )
                output_noise_file_path = gr.Textbox(
                    "",
                    label="output_noise_file 경로",
                    info="path and file to saved output std",
                )
                output_noise_upload = gr.File(
                    label="output_noise.csv 업로드 (선택)", file_types=[".csv"]
                )
                t = gr.Number(1.0, label="t (retention time)", info="retention time")
                v = gr.Number(
                    0.0, label="v (drift coefficient)", info="drift coefficient"
                )
                detect = gr.Radio(
                    [0, 1],
                    value=0,
                    label="detect (0=random, 1=fixed)",
            ...
            with gr.Tab("Quantization"):
                weight_precision = gr.Number(
                    8,
                    label="weight_precision",
                    precision=0,
                    info="number of bits to quantize the weights to (integer)",
                )
                input_precision = gr.Number(
                    8,
                    label="input_precision",
                    precision=0,
                    info="number of bits to quantize the inputs to (integer)",
                )
                dac_precision = gr.Number(
                    1,
                    label="dac_precision",
                    precision=0,
                    info="DAC precision (e.g. 1-bit)",
                )
                adc_precision = gr.Number(
                    7,
                    label="adc_precision",
                    precision=0,
                    info="ADC precision (e.g. 7-bit)",
                )
                input_calib_method = gr.Dropdown(
                    ["max", "histogram"],
                    value="max",
                    label="input_calib_method",
                    info="histogram or max",
                )
                weight_calib_method = gr.Dropdown(
                    ["max", "histogram"],
                    value="max",
                    label="weight_calib_method",
                    info="histogram or max",
                )
                adc_calib_method = gr.Dropdown(
                    ["max", "histogram"],
                    value="max",
                    label="adc_calib_method",
                    info="histogram or max",
                )
                adc_quant_method = gr.Textbox(
                    "scale",
                    label="adc_quant_method",
        ...
        run_btn.click(
            run_inference, inputs=run_inputs, outputs=[log_box, log_dl, result_dl]
        )
...
if __name__ == "__main__":
    demo.launch(
        ...
        server_port=7860,
        share=False,
        auth=_parse_accounts(),
        auth_message="NeuroSim — 로그인이 필요합니다.",
    )
