...
def build_loaders(data_root: str, batch_size: int, num_workers: int):
    # Match normalization / augmentation used in dataset.py for CIFAR-10
...
def parse_args():
    ...
    p.add_argument(
        "--output",
        default="./models/vgg8_cifar10.pth",
        help="Output checkpoint path (must match quantize.py naming if you want inference.py to find it)",
    )
    return p.parse_args()
