...
from models import lenet, resnet, vgg
...
def quantize_model(args, criterion, data_loader, data_loader_test):
    ...
    input_desc = QuantDescriptor(
        calib_method=args.input_calib_method,
        num_bits=args.input_precision,
        fake_quant=args.fake_quant,
        axis=args.input_axis,
        unsigned=False,
    )

    weight_desc = QuantDescriptor(
        calib_method=args.weight_calib_method,
        num_bits=args.weight_precision,
        fake_quant=args.fake_quant,
        axis=args.weight_axis,
        unsigned=False,
    )

    adc_quant_desc = QuantDescriptor(
        calib_method=args.adc_calib_method,
    ...
    evaluate(model, args, criterion, data_loader_test, num_batches=1, print_freq=1)
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for i, (images, labels) in tqdm(enumerate(data_loader), total=num_batches):
            ...
            print(
                "Test Accuracy of the model on {} test images: {} %".format(
        ...
        args.logger(
            "Test Accuracy of the model on the 50000 test images: {} %".format(
                100 * correct / total
            )
        )
